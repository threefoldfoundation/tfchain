...
func TestValidateUniquenessOfNetworkAddresses_Correct(t *testing.T) {
	testCases := [][]tbtypes.NetworkAddress{
		{},
...
func TestValidateBotSignature_Correct(t *testing.T) {
	...
	defer types.RegisterTransactionVersion(tbtypes.TransactionVersionBotRegistration, nil)
	...
	// taken from devnet
	err = validateBotSignature(tx, publicKey, signature, types.TransactionValidationContext{
		ValidationContext: types.ValidationContext{
			Confirmed:   true,
			BlockHeight: 575,
			BlockTime:   1545236350,
		},
	}, tbtypes.BotSignatureSpecifierSender)
	if err != nil {
		t.Fatal(err)
	}
}
...
func mustNewNetworkAddress(t *testing.T, str string) tbtypes.NetworkAddress {
	t.Helper()
	addr, err := tbtypes.NewNetworkAddress(str)
	if err != nil {
		t.Fatal(err, str)
	}
	return addr
...
func mustNewBotName(t *testing.T, str string) tbtypes.BotName {
	t.Helper()
	name, err := tbtypes.NewBotName(str)
	if err != nil {
		t.Fatal(err, str)
	}
	return name
