...
- It is ensured that all names (used by 3Bots) are unique and owned by a single public key;
- It is guaranteed that the required fees are paid without the need of a central service, third party financial service or even fiat currency;
...
- **Public Key**: The unique Public Key (the [ed25519][ed25519] algorithm is the only supported one for the initial deployment of this feature) that is used by the 3Bot to proof that it has the authority to change its record, as to be able to make any future updates as well as the initial registration;
- **Expiration Epoch Time**: Expiration Epoch Time, defining until when the names for a given 3Bot are active/claimed. Beyond this Epoch time the names will still be stored in the record, but should be seen as inactive by the consumer of this data (e.g. 3Bot DNS services). This implies also that when a 3Bot is expired, that any 3Bot (including this 3Bot) can (re)claim the expired names;
    - Note that the record of an expired 3Bot might still contain the names as defined by that 3Bot prior to expiring, even though the 3Bot no longer owns these names. Therefore it is very important that any service sitting on top of a 3Bot record DB checks the expiration date prior to consumption;
...
Note that a single 3Bot will get a unique ID assigned only once, at the point of registration. Once defined it isn't changed or decoupled from the public key (unique to that 3Bot as well), no matter what or how many updates it receives.
...
- an inactive 3Bot can be activated again (by extending the number of months);
    ...
    - this also has as effect that all names that were still registered in the inactive 3Bot's record up to that point get implicitly removed;
- one or multiple name(s) can be added (if the name is available and the bot has less than 5 names after applying the names to-be removed);
- one or multiple name(s) can be removed (only if the 3Bot owns these names);
- one or multiple network address(es) can be added (if the bot has less than 10 addresses after applying the addresses to-be removed);
- one or multiple network address(es) can be removed (if the bot has these addresses registered);

A 3Bot (record) cannot be deleted (the blockchain never forgets, unless it forks). You can however deactivate it, by ensuring all network addresses are removed. No refunds are given. Should you want you can also remove all (DNS) names to free them up already (again no refunds are given), otherwise they'll expire once the record's Expiration Epoch time has been reached.
...
A 3Bot can register one name and up to 10 network addresses free of charge. Modifying addresses or adding names post-registration is never free however. At any given block height, a 3Bot is only allowed up to 5 names and 10 network addresses.
...
As you can see, the difference between 12 months and 24 months is pretty small, making it pretty attractive to sign up immediately for a 2 year period. While saving you a lot of coins, it doesn't lock you to a specific (set of) name(s), as this information (as well as the network addresses used) can still be changed, without affecting the activity period of the 3Bot (or its (to be) paid fees).
...
- At any _resulting_ point no more than 5 names can be registered for a single 3Bot (_resulting_ meaning that if you update a 3Bot that already has 4 names you can add 2 names ONLY if you also remove 1 in that same update Tx);
- At any _resulting_ point no more than 10 network addresses can be registered for a single 3Bot (_resulting_ meaning that if you update a 3Bot that already has 9 addresses you can add 2 addresses ONLY if you also remove 1 in that same update Tx);
- All names have to be valid (more about this later);
...
- The signature has to be valid:
  ...
  - the signature is signed using the private key paired with the known/given public key (only at registration the public key is given);
...
> - By manually computing the required fee;
> - By using the following algorithm: `sum(inputs) - refundOutput - minterFee`;
>
...
> (as unspend output) in order to know how much is
> is to be transferred as a blockchain-generated coin output
> to the Threefold Foundation. By ruling this option out,
> and only accepting the exact required amount to be paid,
> we can quickly compute the fee ourself, using nothing
...
Each name has to be formatted according to the following rules:
...
A name can only be registered if it is available:
- a name is available if it was never registered;
...
- the last 3Bot that owned it removed the name explicitly;
- the name is transferred to the 3Bot registering the name;
